/* eslint-disable unicorn/prefer-add-event-listener,@typescript-eslint/ban-ts-comment */
/**
...
const maxRetriesTest = (count: number, done: () => void) => {
	...
	ws.addEventListener('error', () => {
		...
		if (ws.retryCount > count) {
			throw new Error(`too many retries: ${ws.retryCount}`);
		}
	});
};
...
testDone(
	...
	(done, fail) => {
		...
		ws.addEventListener('close', error => {
			...
			if (error.code === codes[1] && error.reason === String(codes[1])) {
